...
const zooAnimals = [
  ...
  */
  ...
  function lowerCaseNames(zoo){
    return zoo.map((items)=>{ 
  ...
  */
  ...
  function USApop(call){
    const totaluspop = call.reduce((accumulator, index) => accumulator + index['population'],0);
      return totaluspop;
  }
...
function greeting(first, last){
  ...
  console.log(greeting('james','freund'));
...
CuboidMaker.prototype.height = function(height){
  if(this.height.length = 0){
    return this.height.push(height);
  }
}
...
/* 🐴🐴🐴 Step 4: Create a new object that uses CuboidMaker 🐴🐴🐴
  ...
  const vol = new CuboidMaker(4,5,5);
